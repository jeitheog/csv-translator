...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if 'store' not in body or 'token' not in body:
            self._respond(400, {'error': 'Faltan campos: store, token'})
            return

        store = body['store'].strip().replace('https://', '').replace('http://', '').rstrip('/')
        if not store.endswith('.myshopify.com'):
            store += '.myshopify.com'
        token = body['token'].strip()

        status, data = _shopify_request(store, token, 'GET', 'shop.json')
        ...
        if status == 200:
            ...
            self._respond(status or 500, {'success': False, 'error': f"Error ({status}): {error_msg}"})
